...
- [x] `iaw create --recording` + `recorder.py`: grava janelas ativas e **screenshots** (Windows/ctypes+mss, WSL/powershell.exe, Linux/xdotool+mss); `iaw finish-task` fecha a task com resumo por visão (Gemini/agy), anexa até 5 prints e limpa os artefatos (intervalo configurável em `recording_shot_interval`)
